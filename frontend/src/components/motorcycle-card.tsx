...
import { Cog, DollarSign, Calendar, Info, Eye, ImageOff } from 'lucide-react';
...
export default function MotorcycleCard({ motorcycle, large, largeWidth }: MotorcycleCardProps) {
  ...
  return (
    ...
    >
      <CardHeader className="p-0">
        <div className={`relative ${largeWidth ? 'aspect-[4/2.5] min-h-[160px] max-h-[220px]' : 'aspect-video'} rounded-t-xl overflow-hidden` }>
          {motorcycle.imageUrl && motorcycle.imageUrl.trim() ? (
            <Image
              ...
              unoptimized
              onError={(e) => {
                console.error(`Error loading image for ${motorcycle.brand} ${motorcycle.model}:`, motorcycle.imageUrl);
      ...
      <CardContent className="flex-grow p-4 pt-2">
        <div className="space-y-2 text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="truncate">{motorcycle.engine}</span>
          </div>
